...
void setup() {
  ...
  bpmGlide = new Glide(ac, 1.0, 1);
  tempGlide = new Glide(ac, 1.0, 1);
...
public void stressLevel(float value) {
  ...
  bpmPriority = abs(0.5 - bpmGlide.getValue());
...
public void tempLevel(float value) {
  ...
  priority = 0 + bpmPriority + tempPriority;
  tempPriority = abs(0.5 - tempGlide.getValue());
...
//----------------------------------Play
public void play() {
 if (bpmPriority > 0.16 && tempPriority > 0.16
     || priority >= 0.5) {
   notificationHi.setPosition(0);
   notificationHi.start();
 } else if (bpmPriority > 0.16 || tempPriority > 0.16) {
 ...
 delay(1000);
 if (playStress && bpmGlide.getValue() > 0.666) {
   bpmHi.setPosition(0);
   bpmHi.start();
 } else if (playStress && bpmGlide.getValue() < 0.333) {
   bpmLo.setPosition(0);
   bpmLo.start();
 }
 delay(1000);
 if(playTemp && tempGlide.getValue() > 0.666) {
   tempHi.setPosition(0);
   tempHi.start();
 } else if (playTemp && tempGlide.getValue() < 0.333) {
